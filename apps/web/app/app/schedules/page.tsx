...
export default function SchedulesPage(){const router=useRouter(),{hasFullContext}=useOperationalContext();const[rows,setRows]=useState<Schedule[]>([]),[state,setState]=useState<'loading'|'ready'|'error'>('loading'),[error,setError]=useState(''),[page,setPage]=useState(1),[total,setTotal]=useState(0),[from,setFrom]=useState(localDay()),[to,setTo]=useState(localDay(7)),[status,setStatus]=useState('');
 ...
 const columns:DataTableColumn<Schedule>[]=[{key:'when',header:'Horário',render:r=><span className="font-medium text-slate-900">{formatTime(r.starts_at)}{r.ends_at?` — ${new Date(r.ends_at).toLocaleTimeString('pt-BR',{hour:'2-digit',minute:'2-digit'})}`:''}</span>},{key:'customer',header:'Cliente',render:r=>r.customer_name},{key:'os',header:'OS',render:r=>r.service_order_number?`#${r.service_order_number}`:'Sem OS'},{key:'responsible',header:'Responsável',render:r=>r.responsible_name??'Não atribuído'},{key:'status',header:'Situação',render:r=><StatusBadge tone={r.status==='scheduled'?'success':'neutral'}>{r.status==='scheduled'?'Agendado':'Cancelado'}</StatusBadge>}];
 return <RequireOperationalContext><div className="flex flex-col gap-6"><PageHeader title="Agenda operacional" description="Atendimentos programados por filial e responsável." action={<Link href="/app/schedules/new" className="flex items-center gap-2 rounded-xl bg-blue-600 hover:bg-blue-700 px-4 py-2.5 text-sm font-semibold text-white"><PlusCircle className="h-4 w-4"/>Novo agendamento</Link>}><div className="flex flex-wrap gap-3"><label className="text-xs text-slate-600">De<input aria-label="Data inicial" type="date" className="ml-2 rounded-xl border border-slate-300 bg-white p-2 text-sm" value={from} onChange={e=>setFrom(e.target.value)}/></label><label className="text-xs text-slate-600">Até<input aria-label="Data final" type="date" className="ml-2 rounded-xl border border-slate-300 bg-white p-2 text-sm" value={to} onChange={e=>setTo(e.target.value)}/></label><select aria-label="Situação" value={status} onChange={e=>setStatus(e.target.value)} className="rounded-xl border border-slate-300 bg-white px-3 text-sm"><option value="">Todas</option><option value="scheduled">Agendados</option><option value="canceled">Cancelados</option></select></div></PageHeader><DataTable columns={columns} rows={rows} rowKey={r=>r.id} state={state} onRowClick={r=>router.push(`/app/schedules/${r.id}`)} onRetry={()=>load(page)} errorMessage={error} emptyState={<EmptyState icon={CalendarClock} title="Nenhum atendimento no período" description="Ajuste o período ou crie o primeiro agendamento."/>}/><DataTablePagination page={page} pageSize={PAGE_SIZE} total={total} onPageChange={load}/></div></RequireOperationalContext>;
}
