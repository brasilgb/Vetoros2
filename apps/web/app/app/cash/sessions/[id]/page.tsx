  ...
  return <RequireOperationalContext>
    <div className="cash-closing-report flex flex-col gap-5">
      <div className="print-hidden flex flex-wrap items-center justify-between gap-3">
        <button onClick={() => router.push('/app/cash')} className="rounded-xl border border-slate-300 px-4 py-2 text-sm">Voltar ao caixa</button>
        {report?.status === 'closed' && <button onClick={() => window.print()} className="rounded-xl bg-blue-600 hover:bg-blue-700 px-4 py-2 text-sm font-semibold text-white">Imprimir fechamento</button>}
      </div>
      {error && <p role="alert" className="rounded-xl border border-red-300 p-4 text-red-700">{error}</p>}
      {!report && !error && <p>Carregando fechamento…</p>}
      {report && <>
        <section className="rounded-2xl border border-slate-200 bg-slate-50 p-6">
          <p className="text-xs uppercase tracking-widest text-slate-500">{report.status === 'closed' ? 'Comprovante de fechamento de caixa' : 'Conferência provisória — sessão aberta'}</p>
          ...
          <p className="mt-3 font-mono text-xs text-slate-500">Sessão {report.id}</p>
        </section>
        <section className="grid grid-cols-1 gap-4 rounded-2xl border border-slate-200 p-6 sm:grid-cols-2">
          <div><p className="text-xs uppercase text-slate-500">Abertura</p><p>{formatDateTime(report.opened_at)}</p><p className="text-sm">Operador: {report.opened_by_name ?? 'Não identificado'}</p></div>
          <div><p className="text-xs uppercase text-slate-500">Fechamento</p><p>{report.closed_at ? formatDateTime(report.closed_at) : 'Sessão ainda aberta'}</p><p className="text-sm">Operador: {report.closed_by_name ?? '—'}</p></div>
        </section>
        <section className="rounded-2xl border border-slate-200 p-6">
          ...
          ].map(([label, value]) => <div key={String(label)}><p className="text-xs uppercase text-slate-500">{label}</p><p className={label === 'Diferença' && Number(value) !== 0 ? 'font-semibold text-amber-600' : 'font-medium'}>{value === null ? '—' : formatCurrency(value)}</p></div>)}</div>
          <p className="mt-4 text-sm">Quantidade de movimentos: {report.summary.movement_count}</p>
          {report.summary.difference !== null && Number(report.summary.difference) !== 0 && <div className="mt-4 rounded-xl border border-amber-200 p-4"><p className="font-semibold">Divergência: {formatCurrency(report.summary.difference)}</p><p className="mt-1 text-sm">Justificativa: {report.closing_justification || 'Não informada (fechamento histórico anterior ao CAI-03)'}</p><p className="text-sm">Responsável: {report.closed_by_name ?? 'Não identificado'}</p></div>}
        </section>
        <section className="rounded-2xl border border-slate-200 p-6">
          <h2 className="font-semibold">Formas de pagamento</h2>
          <p className="mt-1 text-xs text-slate-500">Composição dos recebimentos; os métodos não representam necessariamente dinheiro físico.</p>
        ...
        <p className="text-xs text-slate-500">Relatório regenerado a partir dos fatos canônicos da sessão. Emitido em {formatDateTime(new Date())}.</p>
      </>}
    </div>
