...
export function AppHeader({ onOpenMobileMenu }: { onOpenMobileMenu: () => void }) {
  ...
  const selectClass = (highlight: boolean) =>
    `hidden max-w-[10rem] truncate rounded-lg border bg-white px-2.5 py-1.5 text-xs text-slate-900 disabled:opacity-40 sm:block ${
      highlight ? 'animate-pulse border-amber-500 ring-2 ring-amber-500/50' : 'border-slate-200'
    }`;

  return (
    <header className="sticky top-0 z-30 flex h-16 shrink-0 items-center gap-3 border-b border-slate-200 bg-white/80 px-4 backdrop-blur sm:px-6">
      <button aria-label="Abrir menu" onClick={onOpenMobileMenu} className="rounded-lg p-2 text-slate-500 hover:bg-slate-100 md:hidden">
      ...
      <div className="ml-auto flex items-center gap-2">
        {missingContext && (
          <span className="hidden items-center gap-1 text-xs font-medium text-amber-700 sm:inline-flex" role="status">
        ...
        <DropdownMenu trigger={<User className="h-[18px] w-[18px]" />} triggerLabel="Menu do usuário" triggerClassName="flex h-9 w-9 items-center justify-center rounded-full bg-slate-100 text-slate-700 hover:bg-slate-200">
